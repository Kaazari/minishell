...
/*   Updated: 2025/08/03 17:30:43 by kclaudan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				handle_logical_commands(t_shell *shell, t_cmd **commands,
						int count);
void				handle_piped_commands(t_shell *shell, t_cmd **commands,
						int count);
void				process_command(t_shell *shell, char *input);
int					init_pipex_for_commands(t_shell *shell, t_cmd **commands,
						int count);
void				cleanup_pipex_after_execution(t_shell *shell);
int					should_continue_logical(t_cmd **commands, int i,
						int prev_exit_status);
int					execute_logical_command(t_shell *shell, t_cmd **commands,
						int i);
...
void				process_final_command(t_cmd_process *proc, int start,
						int i);
...
void				handle_redirection_token(t_token_data *data, char *input,
						int *i);
void				handle_pipe_token(t_token_data *data, int *i);
void				handle_logical_token(t_token_data *data, char *input,
						int *i);
void				handle_pipe_token_cmd(t_cmd_process *proc, int i);
void				handle_logical_token_cmd(t_cmd_process *proc, int i,
						int operator);
...
void				init_tokenize_data(t_token_data *data,
						t_tokenize_init *init);
int					init_tokenize_struct(t_tokenize_init *init,
						char *current_word, int *counts);
char				**handle_quote_error(char **words);
int					check_syntax_errors(char **words);
int					add_variable_to_word(t_var_expansion *var_exp,
						t_shell *shell);
...
void				add_word_to_array(char **words, int *count, char *word,
						int len);
...
int					preprocess_all_heredocs(t_shell *shell, t_cmd **commands,
						int cmd_count);
...
void				finalize_pipe_execution(t_cmd **commands, int cmd_count,
						int prev_fd, pid_t *pids);
...
void				cleanup_parent_pipes(int *prev_fd, int *pipe_fd, int i,
						int cmd_count);
...
int					execute_logical_command(t_shell *shell, t_cmd **commands,
						int i);
void				handle_redirections_context(t_shell *shell,
						int in_pipe_context);
void				restore_redirections_context(t_shell *shell,
						int in_pipe_context);
...
void				handle_variable_expansion(char *input, int *i,
						t_token_data *data, t_shell *shell);
void				handle_backslash(char *input, int *i, t_token_data *data);
void				handle_quote_error_input(char quote_state,
						t_token_data *data);
void				process_variable_expansion(t_var_expansion_context *ctx);
void				process_quote_handling(t_quote_context *ctx);
void				process_backslash_handling(char *input, int *i,
						t_token_data *data, char *quote_state);
...
void				handle_heredoc_error(int *pipe_fd, t_cmd *cmd, int i,
						t_shell *shell);
void				handle_heredoc_success(int *pipe_fd, t_cmd *cmd, int i);
void				handle_variable_in_iteration(t_char_iteration_context *ctx,
						int *i);
void				handle_quote_in_iteration(t_char_iteration_context *ctx,
						int *i);
void				handle_backslash_in_iteration(t_char_iteration_context *ctx,
						int *i);
